...
class Scraper:
    ...
    async def run(self):
        ...
        ny_now = now.astimezone(tz=Scraper.ny_tz)
            ...
            for expiration in weekly_monthly_expirations:
                for ex_date in expiration['expiration_dates']:
                    ...
                    for option_data in options_res['data']:
                        ...
                        # transform fields
                        if self.transform_fields(uncameled_option_data) is None:
    ...
    def transform_fields(self, uncameled_option_data):
        try:
            trade_time = uncameled_option_data['trade_time']
            uncameled_option_data['trade_time'] = datetime.datetime.fromtimestamp(trade_time, tz=Scraper.ny_tz) if trade_time > 0 else datetime.datetime.fromtimestamp(0)
        except Exception as e:
...
#     pass
#
# MyManager.register('RateLimit', RateLimit)
